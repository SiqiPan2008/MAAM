...
\begin{document}
	...
	First, we acquire data from publicly available online datasets. For abnormities that we have no data of, we search for images on Google Image. The numbers of images acquired from each data source are shown in Tables \ref{tb:OCT_source} and \ref{tb:Fundus_source}. For the links if the images from Google Image, please refer to ``Appendix''. 
	...
	Then, for OCT images that have less than 1000 images, we use Cycle-GAN to generate new images. We train a Cycle-GAN network to interconvert healthy and abnormity images, take out the generator that converts healthy images to abnormity images, and use it to generate new abnormity images. We manually eliminate generated images that look obviously wrong. The elimination rate is shown in Table~\ref{tb:cycleGAN_number}. We only used Cycle-GAN to generate OCT images because generated fundus images are too blurry and do not display the target abnormities.
	
	{
		\fontsize{9}{12}\selectfont
		{
			\begin{longtable}{ccccc}
				\caption{OCT images CycleGAN elimination rate}
	...
	We separate the images, including generated images, into two datasets, one training and the other for testing. We implement a set of transformations on the images, including a horizontal flip, a random brightness change from -10\% to +10\%, random horizontal and vertical translations between -5\% and +5\%, random scaling between -20\% and +20\%, and a random rotation, which is between -10$^\circ$ and +10$^\circ$ for OCT images and between -30$^\circ$ and +30$^\circ$ for Fundus images. The rotation is only applied for images in the train dataset. Eventually, we get 5000 train images and 500 test images for each OCT abnormity, and 3000 train images and 300 test images for each fundus abnormity.
