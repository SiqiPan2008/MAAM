...
\begin{document}
	...
	Table~\ref{tb:OCT_test} and Table~\ref{tb:Fundus_test} show the predictive values obtained when the Abnormity Models are tested on the test dataset. Fig.~\ref{fig:A_ROC} shows the ROCs of each abnormity. From the tables and the figure, we can see that the performance of the OCT Model is significantly better than that of the Fundus Model, likely because fundus abnormities tend to be subtler and there are more fundus images that have multiple abnormities. 
	...
	Fig.~\ref{fig:gradCAM_multi_abnormity} demonstrates that the Abnormity Models may be able to correctly identify multiple abnormities on an image. Knowning that OCT and fundus images often do include more than one abnormities, we employ a method to determine the codominant abnormities. We input an image to the Abnormity Models and get the abnormity probabilities vector, and we take the highest probability.   050AO78.017\%, 152AF71.889\%
